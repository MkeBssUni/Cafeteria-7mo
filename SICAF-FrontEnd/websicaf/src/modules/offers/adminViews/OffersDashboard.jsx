...
import UpdateDiscountByTotalOrder from "../adminViews/UpdateDiscountByTotalOrder";
import UpdateDiscountByRol from "./UpdateDiscountByRol";
import UpdateDiscountByProduct from "./UpdateDiscountByProduct";
import UpdateDiscountByCategory from "./NewDiscountByCategory";
import UpdateDiscountByNumberOfProducts from "./UpdateDiscountByNumberOfProducts";
...
const OffersDashborard = () => {
  ...
  const [UpdatemodalShowByrol, setUpdateModalShowByRol] = useState(false);
  const [UpdatemodalShowByCategory, setUpdateModalShowByCategory] =
    useState(false);
  const [UpdatemodalShowBytotal, setUpdateModalShowBytotal] = useState(false);
  const [
    UpdatemodalShowByNumberOfProducts,
    setUpdatemodalShowByNumberOfProducts,
  ] = useState(false);
  const [UpdatemodalShowByProducts, setUpdatemodalShowByProducts] =
    useState(false);

  const getModalUpdate = (discount) => {
    console.log(discount);
    setSelectedDiscount(discount);
    switch (discount.type) {
      case "Descuento por rol":
        setUpdateModalShowByRol(true);
        break;
      case "Descuento por cantidad de productos":
        setUpdatemodalShowByNumberOfProducts(true);
        break;

      case "Descuento por total de la compra":
        setUpdateModalShowBytotal(true);
        break;
      case "Descuento por producto":
        setUpdatemodalShowByProducts(true);
        break;

      case "Descuento por categoria":
        setUpdateModalShowByCategory(true);
        break;
    }
  };
  ...
  const getDiscountByType = async () => {
    ...
    switch (category) {
      case "Todos":
        ...
        console.log(descuentos,'todos los descuentos')
    ...
    <>
      <body>
        ...
        >
          ...
          <Form.Select
            className="input-search text-center mx-4"
            onChange={({ target }) => {
          ...
          >
            ...
            <option value="Descuento por total de compra">
              Descuento por total de compra
            </option>
            <option value="Descuento por categoria">
              Descuento por categoria
            </option>
            <option value="Descuento por cantidad de productos">
              Descuento por cantidad de productos
            </option>
            <option value="Descuento por producto">
              Descuento por producto
            </option>
          </Form.Select>

          <Col xs="12" sm="12" md="1" lg="3" xlg="3">
            <Dropdown>
              <Dropdown.Toggle className="input-search">
              ...
              <Dropdown.Menu className="input-search">
                ...
                <Dropdown.Item
                  as={Button}
                  eventKey="3"
                  onClick={() => setNumberProouct(true)}
                >
                  Descuento cantidad de producto
                </Dropdown.Item>
                <Dropdown.Item
                  as={Button}
                  eventKey="4"
        ...
        {UpdatemodalShowBytotal && (
          <UpdateDiscountByTotalOrder
            show={UpdatemodalShowBytotal}
            onHide={() => setUpdatemodalShowBytotal(false)}
            product={selectedDiscount}
          />
        )}

        {UpdatemodalShowByCategory && (
          <UpdateDiscountByCategory
            show={UpdatemodalShowByCategory}
            onHide={() => setUpdateModalShowByCategory(false)}
            product={selectedDiscount}
          />
        )}

        {UpdatemodalShowByNumberOfProducts && (
          <UpdateDiscountByNumberOfProducts
            show={UpdatemodalShowByNumberOfProducts}
            onHide={() => setUpdatemodalShowByNumberOfProducts(false)}
            product={selectedDiscount}
          />
        )}

        {UpdatemodalShowByrol && (
          <UpdateDiscountByRol
            show={UpdatemodalShowByrol}
            onHide={() => setUpdatemodalShowByrol(false)}
            product={selectedDiscount}
          />
        )}

        {UpdatemodalShowByProducts && (
          <UpdateDiscountByProduct
            show={UpdatemodalShowByProducts}
            onHide={() => setUpdatemodalShowByProductsl(false)}
            product={selectedDiscount}
          />
        )}
        {/* <RegisterDiscount show={modalShow} onHide={() => setModalShow(false)} /> */}
        <RegisterDiscountForRol
          show={modalShow}
          onHide={() => setModalShow(false)}
        />
        <RegisterDiscountByOrderTotal
          show={byOrderTotal}
          onHide={() => setByordertotal(false)}
        />
        <NewDiscountByCategory
          show={byCategory}
          onHide={() => setbyCategory(false)}
        />
        <NewDiscountByNumberOfProducs
          show={numberProouct}
          onHide={() => setNumberProouct(false)}
        />
        <NewDiscountByProduct
          show={byProduct}
          onHide={() => setbyProduct(false)}
        />
                        ...
                        <Row>
                          <Col xs={12} md={3}>
                            <div className="imagewithoffer mt-2">
                              <Image
                                className="image_product_offers my-2 shadow"
                                src={discount.image ? discount.image : logo}
                                roundedCircle
                              />
                              <div className="notification-icon shadow">
                                -{discount.percentage}%
                              </div>
                            </div>
                          </Col>
                          <Col xs={12} md={8} className="mx-2 my-2">
                            <div>
                              <p className="info_products_offers mt-2 ms-4">
                              ...
                              <Row>
                                <Col className="text-center">
                                  <Button
                                    ...
                                    onClick={() => {
                                      getModalUpdate(discount);
                                    }}
                                  >
                                    Editar
                                    <FeatherIcon icon="edit-3" size={17} />
                                  </Button>
                                </Col>
                                <Col>
                                  <OverlayTrigger
